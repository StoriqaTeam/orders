...
pub fn make_router() -> RouteParser<Route> {
    ...
    route_parser.add_route_with_params(r"^/cart/by-session/(\S+)$", |params| {
        params
            .get(0)
    ...
    route_parser.add_route_with_params(r"^/cart/by-session/(\S+)/products/(\d+)$", |params| {
        if let Some(customer_id_s) = params.get(0) {
            if let Some(product_id_s) = params.get(0) {
                if let Ok(customer) = customer_id_s.parse().map(SessionId).map(CartCustomer::Anonymous) {
                    if let Ok(product_id) = product_id_s.parse().map(ProductId) {
                        return Some(Route::CartProduct { customer, product_id });
                    }
                }
    ...
    route_parser.add_route_with_params(r"^/cart/by-session/(\S+)/products/(\d+)/increment$", |params| {
        if let Some(customer_id_s) = params.get(0) {
            if let Some(product_id_s) = params.get(0) {
                if let Ok(customer) = customer_id_s.parse().map(SessionId).map(CartCustomer::Anonymous) {
                    if let Ok(product_id) = product_id_s.parse().map(ProductId) {
